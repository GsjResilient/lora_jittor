...
class Attention(nn.Module):
    ...
    def execute(self, x, history=None, layer_past=None, len_past=None):
        hidden_states = x
        # print(type(x) ,'=' * 50)
        # print(x.shape,x.weight.shape,'!!!'*100)
        x = self.c_attn(x)
        ...
        if layer_past is not None:
            ...
            if len_past is None:
                # print(layer_past.shape,'摇起来'*100)
...
class GPT2Model(nn.Module):
    def __init__(self, config):
        ...
        # print(config.n_embd, '那尼？' * 50)
        self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        self.wpe = nn.Embedding(config.n_positions, config.n_embd)

        block= Block(config.n_ctx, config, scale=True)
        # for n,p in block.named_parameters():
        #     print(n,p.is_stop_grad(),'ddd'*50)
    ...
    def execute(
        ...
        # print(input_ids.shape,'qian'*100)
        input_ids = input_ids.view(-1, input_ids.size(-1))
        position_ids = position_ids.view(-1, position_ids.size(-1))
        # print(type(jt.array(input_ids)),type(input_ids),'baby'*100)
        # print(input_ids.shape, 'input' * 50)
...
class GPT2LMModel(nn.Module):
    def __init__(self, config):
        ...
        #     print(n,p.is_stop_grad(),'ccccccccccc')
    ...
    def execute(
        ...
        # print(len(input_ids), input_ids.shape, 'here' * 100)

        #transformer的问题！！！！
        # print(input_ids, '希望你不是高手')
        hidden_states, presents = self.transformer(input_ids, past=past, len_past=len_past)
        # print(len(presents), '我在这里！！')
        # batch, seq, vocab
        # print(hidden_states.shape, '你是高手么？')
    ...
    def load_weight(self, state_dict):
        # for n,p in self.transformer.named_parameters():
            # print(n,p.is_stop_grad(),p.shape,'44')
        if 'model_state_dict' in state_dict:
            print("YES model")
            state_dict = state_dict['model_state_dict']
        # for _ in state_dict.items():
        #     print(_[0],_[1].shape)
        state_dict_tmp = copy.deepcopy(state_dict)
        # for _ in state_dict_tmp.items():
        #     print(_[0],_[1])
        #     break
        old_keys = []
        new_keys = []
        ...
        for old_key, new_key in zip(old_keys, new_keys):
            # print(new_key,old_key,state_dict[old_key],'='*100)
            state_dict[new_key] = state_dict.pop(old_key)
            # print(type(new_key), type(state_dict[new_key]))
        cnt = 0

        # for _ in state_dict.items():
        #     print(_[0],_[1].shape,'hi')
        # print(len(state_dict))
        for n, p in self.transformer.named_parameters():
            # cnt+=1
            # print(n)
            # print(n,p.is_stop_grad())
            # if p.is_stop_grad():
            #     print(n)
        ...
        # print('finish')
        # print(type(state_dict),'='*50)
        # filtered_state_dict = {k: v for k, v in state_dict.items() if k in self.transformer.state_dict()}
        # print("YES2")
        # print(len(state_dict))
        # lst=[]
        # with open("output.txt", "r", encoding="utf-8") as f:
        #     dataa = [line.strip() for line in f]  # 去除换行符
        # for i in dataa:
        #     print(i,'???')
        ...
        #     print(n,p.shape)
        # for _ in state_dict.items():
        #     print(_[0],_[1].shape,idd)
        #     idd+=1
        # ttt="h.0.attn.c_attn.weight"
        # self.test(ttt)
        # if hasattr(self.transformer,ttt):
        #     print(self.transformer.__getattr__(ttt))
        # for n, p in self.transformer.named_parameters():
        #     print(n, p.shape)
