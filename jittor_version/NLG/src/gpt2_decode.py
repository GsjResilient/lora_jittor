...
if __name__ == "__main__":
    ...
    with open(args.sample_file, 'r') as sample_reader, \
        ...
        line_id = 0
        print()
        # print(len(input_reader.readlines()),'怎么个事')
        for line in input_reader:
            ...
            line_id += 1
        # print(context_list,'荒谬！')
        # for idx, _ in enumerate(refer_dict.keys()):
        #     print(idx, _)
        ...
        # print(len(refer_dict.keys()),'refer字典的长度')
        references = [refer_dict[s]['references'] for s in refer_dict]
        cnt=0
        # print(refer_dict.keys())
        # for s in refer_dict:
        #     if 'sample' not in refer_dict[s].keys():
        #         cnt+=1
        #     else:
        #         print(s)
                # print(s,'没有')
        # print(cnt,len(refer_dict))
        ...
        if args.ref_type == 'e2e':
            with open(args.output_ref_file, 'w', encoding='utf8') as ref_writer:
                # print(len(references),len(hypothesis),'写入')
