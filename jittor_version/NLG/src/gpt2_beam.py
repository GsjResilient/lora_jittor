...
def _reorder_cache(past: Tuple, beam_idx: jt.Var) -> Tuple[jt.Var]:
    ...
    # print(jt.reindex(past[0], past[0].shape,indexes, extras=[beam_idx]).shape, past[0].shape, beam_idx.shape, '闹啥子麻！')
        ...
        for idx, data in enumerate(data_iter):
            ...
            with jt.no_grad():
                for i in range(0, args.eval_len):
                    ...
                    # print(past[0].shape, beam_idx.shape, 'look at my eyes!')

                    past = _reorder_cache(past, beam_idx.view(-1))
                    # print(past[0].shape, 'look at my eyes!')
            ...
            output = output.view(-1, output.shape[-1]).cpu() #????output呢？？？
            #score = score.view(-1, score.shape[-1]).cpu()
            # print(_id[0].item(), len(all_predictions), '你是？')
